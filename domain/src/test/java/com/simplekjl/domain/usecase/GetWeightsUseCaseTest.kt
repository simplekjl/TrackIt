...
internal class GetWeightsUseCaseTest {
    ...
    private lateinit var useCase: GetWeightsUseCase

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        useCase = GetWeightsUseCase(dispatcher, repository)
    }

    @Test
    fun `call repository when using usecase`() = runBlockingTest {
        val weightRaw: Weight = fixture()
        every { repository.getAllWeights() } returns listOf(weightRaw)
        val resultFlow = useCase(Unit).first()
        verify { repository.getAllWeights() }
        assert(resultFlow is Result.Success<*>)
    }

    @Test
    fun `call repository returns exception`() = runBlockingTest {
        every { repository.getAllWeights() } throws Exception("")
        val resultFlow = useCase(Unit).first()
        verify { repository.getAllWeights() }
